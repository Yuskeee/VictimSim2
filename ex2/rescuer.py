...
class Rescuer(AbstAgent):
    ...
    def __planner(self):
        """Usa algoritmo genético para definir a ordem de socorro das vítimas."""

        def fitness(individual):
            """Calcula o fitness como o inverso do tempo total para resgatar."""
            total_time = 0
            current_pos = (0, 0)
            ...
            # Adiciona o tempo de retorno à base
            total_time += abs(current_pos[0]) * self.COST_LINE + abs(current_pos[1]) * self.COST_DIAG
            return 1 / (total_time + 1)  # Fitness é o inverso do tempo total
        ...
        def mutate(individual):
            """Mutação aleatória para evitar convergência prematura."""
        ...
        for _ in range(100):
            ...
            new_population = population[:5]  # Elitismo: mantém os 5 melhores
        ...
        # Seleciona o melhor indivíduo após as gerações
        best_individual = population[0]

        # Converte a sequência de vítimas em ações
        self.plan = []
        current_pos = (0, 0)
